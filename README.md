...
| 409 | `CONFLICT` | `The GitHub webhook created for the submitted repository collided with one a different registration already records. The submitted repository is not registered. GitHub assigns a new webhook id on each attempt, so registering again is the first thing to try.` | The submitted repository has never been registered. The collision is on the webhook id GitHub minted for this attempt, and GitHub mints a new one every time, so register the repository again first. Only a collision that repeats points at stored state that has to be resolved. |
