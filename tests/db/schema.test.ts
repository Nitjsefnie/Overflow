...
describe("initial PostgreSQL materialization", () => {
  ...
  it("stores rejected settlement evidence with its closing pull request and removes it when evidence is accepted", async () => {
    ...
    const rejectedState = {
      ...
      githubPullRequestId,
      reason: "No nonblank rationale comment by the repository sponsor's account (login `materialization-sponsor`) was posted between fifteen minutes before the label at 2026-09-01T11:00:00.000Z and fifteen minutes after the merge at 2026-09-01T12:00:00.000Z.",
    };
    const store = new PostgresFoldStore(sql);
...
function authoritativeIssue(input: { id: number; number: number; ownerLogin: string }): GitHubIssue {
  return {
    ...
    history: [
      {
        kind: "LABELED", id: `opening-${input.id}`, actorLogin: input.ownerLogin, actorGitHubUserId: null,
        label: "M", createdAt: "2026-09-01T08:01:00.000Z",
      },
      {
        kind: "LABELED", id: `actual-${input.id}`, actorLogin: input.ownerLogin, actorGitHubUserId: null,
