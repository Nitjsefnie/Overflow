...
export function assertClosingPullRequestQuery(
  ...
  expect(normalized).toContain(`closedByPullRequestsReferences(${argumentsText}) { nodes { databaseId number title body url state mergedAt mergeCommit { oid } commits(last: 1) { nodes { commit { committedDate } } } author { login } } pageInfo { hasNextPage endCursor } }`);
