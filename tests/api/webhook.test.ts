...
describe("GitHub webhook route", () => {
  ...
  it("keeps setup configuration aligned with the production App Router pathname", async () => {
    ...
    const [environmentExample, operating] = await Promise.all([
      readFile(resolve(".env.example"), "utf8"),
      readFile(resolve("OPERATING.md"), "utf8"),
    ]);
    ...
    expect(operating).toContain(`GITHUB_WEBHOOK_URL=https://<public-host>${routePathname}`);
    expect(environmentExample).not.toContain("/api/webhooks/github");
    expect(operating).not.toContain("/api/webhooks/github");
  });
