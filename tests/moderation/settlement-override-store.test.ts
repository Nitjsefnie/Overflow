...
async function insertSettlement(options: { reviewRounds?: number } = {}): Promise<{
  ...
  const [repository] = await sql<{ id: string }[]>`
    ...
    values (
      ${githubRepositoryId}, ${`owner-${githubRepositoryId}/repository`}, ${debtorId},
  ...
  const [pullRequest] = await sql<{ id: string }[]>`
    ...
    values (
      ...
      ${creditorId}, ${creditor.github_login}, ${"MERGED"}, ${"2026-09-01T12:00:00.000Z"},
  ...
  const [settlement] = await sql<{ id: string }[]>`
    ...
    values (
      ${pullRequest.id}, ${issue.id}, ${creditorId}, ${creditor.github_login}, ${debtorId},
