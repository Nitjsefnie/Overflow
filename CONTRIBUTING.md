...
Footer must be one final line in the form
`Generated by <model> (<role>, <role>, ...), <model> (<role>, ...)`.
If Bugs Discovered applies, use only `- #123 Literal issue title` entries:
each number must identify an existing issue in this repository, and each title
must match that issue's GitHub title exactly. Pull-request numbers and narrative
